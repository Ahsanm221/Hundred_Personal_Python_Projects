...
class Executor(object):
            ...
            if self.supports_fancy_output():
                    ...
                    if self._should_write_operation(operation):
                        with self._lock:
                            ...
                            self._sections[id(operation)].write_line(
                                "  <fg=blue;options=make_bold>•</> {message}: <fg=blue>Pending...</>".format(
                ...
                if self._should_write_operation(operation):
                    if not operation.skipped:
                        self._io.write_line(
                            "  <fg=blue;options=make_bold>•</> {message}".format(
                                message=self.get_operation_message(operation),
                            ),
                        )
                    else:
                        self._io.write_line(
                            "  <fg=default;options=make_bold,dark>•</> {message}: "
                            "<fg=default;options=make_bold,dark>Skipped</> "
                            "<fg=default;options=dark>for the following reason:</> "
                            "<fg=default;options=make_bold,dark>{reason}</>".format(
                                message=self.get_operation_message(operation),
    ...
    def _do_execute_operation(self, operation):
        ...
        message = "  <fg=green;options=make_bold>•</> {message}".format(
    ...
    def _execute_uninstall(self, operation):  # type: (Uninstall) -> None
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Removing...</info>".format(
            message=self.get_operation_message(operation),
        )
        self._write(operation, message)
    ...
    def _install(self, operation):
        ...
        operation_message = self.get_operation_message(operation)
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Installing...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _prepare_file(self, operation):
        ...
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Preparing...</info>".format(
            message=self.get_operation_message(operation),
        )
        self._write(operation, message)
    ...
    def _install_directory(self, operation):
        ...
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Building...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _install_git(self, operation):
        ...
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Cloning...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _download_archive(self, operation, link):  # type: (Operation, Link) -> Path
        ...
        operation_message = self.get_operation_message(operation)
        message = "  <fg=blue;options=make_bold>•</> {message}: <info>Downloading...</>".format(
            message=operation_message,
        )
        progress = None
        if self.supports_fancy_output():
